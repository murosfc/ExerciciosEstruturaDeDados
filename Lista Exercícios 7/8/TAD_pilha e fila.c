...
tipo_fila * criarFila ()
{
	...
	filacriar->fim=NULL;
	filacriar->quantidadeElementos=0;
	return filacriar;
}
...
int addFila (tipo_fila * filaAdd, int num)
{
	...
	{
		anterior=filaAdd->fim;
		addaux (filaAdd, novo, num);
		anterior->ponteiro=novo;
	}
	if (novo->dado == num)
		return 1;
	else return 0;
...
void apagarFila (tipo_fila * apaga_fila)
{
	...
	while (apaga_fila->inicio != NULL)
	{
		...
		else
		{
			...
			apaga_fila->fim=NULL;
			apaga_fila->quantidadeElementos =0;
			free (temp);	
		}
	}	
}

int consultaFilaPorProfundidade (tipo_fila * fila_consulta, int profundidade)
{
	int i;
	tipo_no *temp;
	temp=fila_consulta->inicio;
	if (profundidade <= fila_consulta->quantidadeElementos)
	{
		if (profundidade == 0)
			return temp->dado;	
		else
